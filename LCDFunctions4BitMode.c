...
void Lcd_printval(Uint16 rowNumber, Uint16 colNumber, float value, char* unit)
{
    Uint16 i, vallen;
    unsigned char Display, rowCount;
    char buffer[20];
    ...
    intPart = (int)value;
    decPart = (int)((value - intPart) * 100);   // two decimal precision

    if (decPart < 0)
        decPart = -decPart;                     // handle negative decimals

    // Convert number to string: "123.45"
    sprintf(buffer, "%d.%01d", intPart, decPart);

    // Get string length
    vallen = stringLen(buffer);
